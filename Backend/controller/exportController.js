...
// Typography configuration matching the PDF expert
          ...
          paragraphs.push(
            new Paragraph({
              ...
              style: {
                ...
                size: fontSize * 2, // docx uses half-ponts
              },
            })
          );
          ...
          if (textRuns.length > 0) {
            paragraphs.push(
              new Paragraph({
                children: textRuns,
                spacing: {
                  before: inList ? 100 : DOCX_STYLES.spacing.chapterBefore,
                  after: inList ? 100 : DOCX_STYLES.spacing.chapterAfter,
                  line: 360,
                },
...
const processInlineContent = (children) => {
  const textRuns = [];
  let currentFormating = { bold: false, italic: false };
  ...
  const flushText = () => {
    if (textBuffer.trim()) {
      textRuns.push(
        new TextRun({
          text: textBuffer,
          bold: currentFormating.bold,
          italics: currentFormating.italic,
          font: DOCX_STYLES.fonts.body,
  ...
  children.forEach((child) => {
    if (child.type === "strong_open") {
      flushText();
      currentFormating.bold = true;
    } else if (child.type === "strong_close") {
      flushText();
      currentFormating.bold = false;
    } else if (child.type === "em_open") {
      flushText();
      currentFormating.italic = true;
    } else if (child.type === "em_close") {
      flushText();
      currentFormating.italic = false;
...
const exportAsDocument = async (req, res) => {
  try {
    ...
    if (book.coverImage && !book.coverImage.includes("avatar")) {
    ...
    book.chapters.forEach((chapter, index) => {
      try {
        // page beak before each chapter (except first)
        if (index > 0) {
          sections.push(
            new Paragraph({
              text: "",
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    try {
      if (token.type === "heading_open") {
        ...
        doc.markdown(TYPOGRAPHY.spacing.listSpacing / TYPOGRAPHY.sizes.body);
      } else if (token.type === "code_block" || token.type === "fence") {
        doc.moveDown(
          TYPOGRAPHY.spacing.paragraphSpacing / TYPOGRAPHY.sizes.body
        );
...
const exportAsPDF = async (req, res) => {
  try {
    ...
    // create a PDF with safe settings
    const doc = new PDFDocument({
      margins: { top: 72, left: 72, right: 72 },
      bufferPages: true,
      autoFirstPage: true,
    });
